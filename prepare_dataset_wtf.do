...
/***************************
ccode2 numbers for Z-countries 
****************************/

replace ccode2 = 551 if importer == "Zambia"
replace ccode2 = 511 if importer == "Zanzibar"
replace ccode2 = 552 if importer == "Zimbabwe"
replace ccode2 = 552 if importer == "Zimbabwe (Rhodesia)"
drop if ccode==.
drop if ccode2==.
**somma unit
bysort ccode ccode2: egen tot_value=sum(value)


***** creating the dummy for being a net exporter and net exporter to US
bysort ccode: egen tot_export=max(tot_value)
replace ccode=ccode2 if ccode==0
g tot_imp=.
replace tot_imp=tot_value if ccode==ccode2
bysort ccode: egen tot_import=max(tot_imp)
replace tot_import=0 if tot_import==. 
replace ccode=0 if ccode==ccode2

g imp_usa=.
replace imp_usa=tot_value if ccode2==2
bysort ccode: egen import_usa=max(imp_usa)
replace import_usa=0 if import_usa==.

g exp_usa=.
replace ccode=ccode2 if ccode==2
replace exp_usa=tot_value if ccode==ccode2
bysort ccode: egen export_usa=max(exp_usa)
replace export_usa=0 if export_usa==. 
replace ccode=2 if ccode==ccode2&exporter!="World"
...
save wtf00_cleaned.dta,replace
...
collapse year net_exp net_exp_usa tot_export tot_import import_usa export_usa, ///
 ...
 gen net_exp_dummy=net_exp>0
 gen net_exp_usa_dummy=net_exp_usa>0
